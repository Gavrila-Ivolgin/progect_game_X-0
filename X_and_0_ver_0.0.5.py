...
while CHOICE_USER == '1':
    ...
    # в цикл while
    def choosing_bot_move(view, symbol):
        keys_list = list(step.keys())

        if 7 < len(keys_list) <= 9 and 5 in keys_list: return 5  # 1-ый ход - занять центр поля
        if 6 <= len(keys_list) <= 9 and any([view[1][1], view[1][2], view[1][3]]) and (
                view[3][1].strip() == "" and view[3][3].strip() == "") and 7 in step:
            print(Fore.LIGHTBLUE_EX + 'Делаю стратегический ход! {7}' + Style.RESET_ALL)
            return 7
        if 6 <= len(keys_list) <= 9 and any([view[3][1], view[3][2], view[3][3]]) and (
                view[1][1].strip() == "" and view[1][2].strip() == "") and 1 in step:
            print(Fore.LIGHTBLUE_EX + 'Делаю стратегический ход! {1}' + Style.RESET_ALL)
            return 1
    ...
    while step:
        if first_step == username:
            ...
            symbol = '0' if len(step) % 2 == 0 else 'x'
            keys_list = list(step.keys())
            step_bot = choosing_bot_move(view, symbol)
